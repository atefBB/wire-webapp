  ...
  _init(database) {
    return Promise.resolve().then(() => {
      ...
      this.cryptobox.on(cryptobox.Cryptobox.TOPIC.NEW_SESSION, session_id => {
        const {userId, clientId} = z.client.Client.dismantleUserClientId(session_id);
        amplify.publish(z.event.WebApp.CLIENT.ADD, userId, new z.client.Client({id: clientId}));
      });
    });
  }
