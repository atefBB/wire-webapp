...
z.client.ClientRepository = class ClientRepository {
  ...
  getAllClientsFromDb() {
    return this.clientService.loadAllClientsFromDb().then(clients => {
      ...
      for (const client of clients) {
        const ids = z.client.Client.dismantleUserClientId(client.meta.primary_key);
        if (ids.userId && !skippedUserIds.includes(ids.userId)) {
          recipients[ids.userId] = recipients[ids.userId] || [];
          recipients[ids.userId].push(this.clientMapper.mapClient(client));
        }
      }
      return recipients;
    });
   ...
   *
   * @param {string} userId - User ID of the client owner
   * @param {z.client.Client} clientEntity - Client which needs to get updated
  ...
  getClientByUserIdFromDb(userId) {
    return this.clientService.loadAllClientsFromDb().then(clients => {
      return clients.filter(client => z.client.Client.dismantleUserClientId(client.meta.primary_key).userId === userId);
    });
  }
